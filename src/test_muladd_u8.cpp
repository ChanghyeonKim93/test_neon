...
int main() {
    ...
    // NEON version
    timer::tic();
    uint8x16_t va[8], vres; // 8 bits x 16 data unsigned int
    ...
    {
        va[0] = vld1q_u8(ptr_a[0]); ptr_a[0] += step;
        va[1] = vld1q_u8(ptr_a[1]); ptr_a[1] += step;
        va[2] = vld1q_u8(ptr_a[2]); ptr_a[2] += step;
        va[3] = vld1q_u8(ptr_a[3]); ptr_a[3] += step;
        va[4] = vld1q_u8(ptr_a[4]); ptr_a[4] += step;
        va[5] = vld1q_u8(ptr_a[5]); ptr_a[5] += step;
        va[6] = vld1q_u8(ptr_a[6]); ptr_a[6] += step;
        va[7] = vld1q_u8(ptr_a[7]); ptr_a[7] += step;

        vres = vmovq_n_u8(0.0f);

        vres = vaddq_u8(vres,vmulq_u8(va[0],va[1]));
        vres = vaddq_u8(vres,vmulq_u8(va[1],va[2])); 
        vres = vaddq_u8(vres,vmulq_u8(va[2],va[3]));  
        vres = vaddq_u8(vres,vmulq_u8(va[3],va[4]));  
        vres = vaddq_u8(vres,vmulq_u8(va[4],va[5]));  
        vres = vaddq_u8(vres,vmulq_u8(va[5],va[6]));  
        vres = vaddq_u8(vres,vmulq_u8(va[6],va[7]));  
        vres = vaddq_u8(vres,vmulq_u8(va[7],va[0]));
