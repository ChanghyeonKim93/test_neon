...
int main() {
    ...
    for(i = 0; i < n_data/step; ++i)
    {
        ...
        vres = vaddq_u16(vres,vmulq_u16(va[0],va[1]));
        vres = vaddq_u16(vres,vmulq_u16(va[1],va[2])); 
        vres = vaddq_u16(vres,vmulq_u16(va[2],va[3]));  
        vres = vaddq_u16(vres,vmulq_u16(va[3],va[4]));  
        vres = vaddq_u16(vres,vmulq_u16(va[4],va[5]));  
        vres = vaddq_u16(vres,vmulq_u16(va[5],va[6]));  
        vres = vaddq_u16(vres,vmulq_u16(va[6],va[7]));  
        vres = vaddq_u16(vres,vmulq_u16(va[7],va[0]));
